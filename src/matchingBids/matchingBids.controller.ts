...
import { MatchingBidsService } from "./matchingBids.service"
...
@Controller("matchingbids")
export class MatchingBidsController {
    /**
     * Creates an instance of RootController.
     * @param rootService - The RootService instance.
     */
    constructor(private readonly matchingBidsService: MatchingBidsService) {}
    ...
    @Post("query")
    async find(
        @Body() queryParam: QueryParam<MatchingBid>
    ): Promise<Result<ValueFields<MatchingBid>[]>> {
        return await this.matchingBidsService.find(queryParam)
    }
    ...
    async total(
        ...
        return await this.matchingBidsService.total(queryParam)
    }
}
