...
import { MatchingTargetService } from "./matchingTarget.service"
...
@Controller("matchingtarget")
export class MatchingTargetController {
    /**
     * Creates an instance of RootController.
     * @param rootService - The RootService instance.
     */
    constructor(
        private readonly matchingTargetService: MatchingTargetService
    ) {}
    ...
    @Post("query")
    async find(
        @Body() queryParam: QueryParam<MatchingTarget>
    ): Promise<Result<ValueFields<MatchingTarget>[]>> {
        return await this.matchingTargetService.find(queryParam)
    }
    ...
    async total(
        ...
        return await this.matchingTargetService.total(queryParam)
    }
}
