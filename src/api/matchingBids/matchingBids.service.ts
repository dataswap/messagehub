 ...
 */
@Injectable()
export class MatchingBidsService {
    /**
    ...
    async find(
        ...
        return await bgTask.context.datastore.matchingBids.find(
     ...
     */
    async total(queryParam: QueryParam<MatchingBid>): Promise<Result<number>> {
        let bgTask: BackgroundTask
        if (queryParam.network === "calibration") {
            bgTask = calibrationBgTask
        } else {
        ...
        return await bgTask.context.datastore.matchingBids.total(
            queryParam.queryFilter
        )
    }
}
