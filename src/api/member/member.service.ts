...
export class MemberService {
    ...
    async find(
        ...
        return await bgTask.context.datastore.member.find(
     ...
     */
    async total(queryParam: QueryParam<Member>): Promise<Result<number>> {
        let bgTask: BackgroundTask
        if (queryParam.network === "calibration") {
            bgTask = calibrationBgTask
        } else {
        ...
        return await bgTask.context.datastore.member.total(
            queryParam.queryFilter
        )
    }
}
